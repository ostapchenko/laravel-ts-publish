...
- **Relation `only()` / `except()`** — `$this->relation->only([...])` now references the generated model type via `Pick<Model, 'a' | 'b'>` (and `Omit<Model, ...>` for `except()`) whenever the relation resolves to a single model *and* every filtered key is a real database column, keeping the model's `#[TsCasts]`/`@property` refinements instead of losing them to a re-derived inline shape. A multi-model accessor union always expands inline. When the shape has to be expanded inline, `except()` now expands to **database columns only** — see the behavior-change note below.
...
- **Model `toArray()` spreads** — `[...$user->toArray(), 'flag' => true]` inside an array literal types as `Omit<User, 'flag'> & { flag: boolean }`, intersecting the model interface with the literal's own keys instead of collapsing to `unknown[]`. The `Omit<>` is what makes the intersection mean what the PHP means, since a later key wins at runtime but `&` would collapse a collision to `never`. Two caveats, because the arm emits a *reference* to the `{Model}` interface rather than a re-derived shape. `Model::toArray()` is `attributesToArray()` merged with `relationsToArray()`; bare `{Model}` covers the first (columns **and** `$appends` — a model's appended accessors are in `{Model}`, not `{Model}Mutators`) but not the second. So: a relation loaded before the spread is in the JSON payload but not in the type; and `$hidden` columns are stripped at runtime but stay in `{Model}` unless `models.exclude_hidden` is on.
- **`toResource()` / `toResourceCollection()`** — `$model->toResource()` and `$collection->toResourceCollection()` resolve through an explicit `SomeResource::class` argument, a `#[UseResource]`/`#[UseResourceCollection]` attribute, or Laravel's naming convention. The naming-convention guess is now only accepted when this package will actually emit that resource, so a guessed class that is third-party, `#[TsExclude]`d, or outside the scanned directories falls back to `unknown` rather than emitting an import of a file that is never written. Explicitly named resources are never gated — a name you wrote down is a declaration, not a guess.
...
>
> `$this->relation->except([...])` used to expand to every attribute **and** every relation on the related model, minus the excluded keys. It now expands to that model's database columns minus the excluded keys, matching what `Model::except()` actually returns at runtime — `HasAttributes::except()` iterates `getAttributes()`, which never contains a relation and never contains a get-only accessor.
>
> Accessors and relations that used to appear in an `except()`-filtered type are therefore **gone**, and any frontend code reading them will now fail to compile. This only affects the inline-expansion path, and the reference path has **two** conditions, not one: the relation must resolve to a *single* model **and** every filtered key must be a real column. A filter meeting both still emits an `Omit<Model, ...>` reference and is unchanged. A multi-model accessor union — `@return Attribute<Image|User|null, never>` and the like — never reaches `relationFilterModelReference()` at all; `ResourceAstAnalyzer::analyzeRelationFilter()` routes it straight to `resolveFilteredRelationType()`, so every arm expands inline and the change applies in full even when the keys are all columns. The worked example below is exactly that case.
>
> Taken from the workbench output for `$this->last_checked_by?->except(['created_at', 'updated_at'])`, whose `Image` arm reads:
>
> ```diff
> - { id: number; imageable_type: string; …; metadata: unknown[] | null; size_for_humans: string; is_landscape: boolean; aspect_ratio: string | null; extension: string | null; … }
> + { id: number; imageable_type: string; …; metadata: unknown[] | null }
> ```
>
> Everything from `size_for_humans` onward was an accessor; none of it is returned by `Image::except()` at runtime.
>
> One consequence: naming a relation or an accessor in the exclusion list is now a no-op, because that key was never in the set being subtracted from. `only()` is deliberately **not** changed — `HasAttributes::only()` calls `getAttribute()` per named key, which does resolve accessors and relations, so `$this->relation->only(['posts'])` still emits the relation. The two branches diverge exactly the way Eloquent's own do.
...
- **Inline paginators** — a paginator called directly inside the render array, with no intermediate variable (`'teams' => new TeamCollection(Team::query()->paginate(10))`), is now detected and typed as a paginator. Previously only the `$teams = Team::query()->paginate(10)` variable form was recognised, and the inline form produced a *wrong* type rather than a missing one: an unrecognised paginator falls back to "not paginated", so the prop was typed as a bare resource or collection with no pagination wrapper. Both the `new SomeResource(...)` and `SomeResource::collection(...)` forms are covered. A query builder assigned to a variable *before* the paginator call (`$q = Post::query(); … $q->paginate(10)`) is still not followed — that remains a known gap.
