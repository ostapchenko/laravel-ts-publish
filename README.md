...
- **Relation `only()` / `except()`** — `$this->relation->only([...])` now references the generated model type via `Pick<Model, 'a' | 'b'>` (and `Omit<Model, ...>` for `except()`) whenever every filtered key is a real database column, keeping the model's `#[TsCasts]`/`@property` refinements instead of losing them to a re-derived inline shape. When the shape has to be expanded inline instead, `except()` now expands to **database columns only** — see the behavior-change note below.
...
>
> Accessors and relations that used to appear in an `except()`-filtered type are therefore **gone**, and any frontend code reading them will now fail to compile. This only affects the inline-expansion path; a filter whose keys are all real columns still emits an `Omit<Model, ...>` reference and is unchanged.
>
