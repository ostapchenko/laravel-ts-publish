...
>
> `$this->relation->except([...])` used to expand to every attribute **and** every relation on the related model, minus the excluded keys. It now expands to that model's database columns minus the excluded keys, matching what `Model::except()` returns at runtime. `HasAttributes::except()` iterates `getAttributes()`, which holds neither relations nor get-only accessors.
>
> Accessors and relations that used to appear in an `except()`-filtered type are **gone**, so frontend code reading them will fail to compile. Only the inline-expansion path changes. The reference path has **two** conditions, not one: the relation must resolve to a *single* model **and** every filtered key must be a real column. A filter meeting both still emits an unchanged `Omit<Model, ...>`. A multi-model accessor union such as `@return Attribute<Image|User|null, never>` never reaches `relationFilterModelReference()` at all; `ResourceAstAnalyzer::analyzeRelationFilter()` routes it straight to `resolveFilteredRelationType()`, so every arm expands inline and the change applies in full even when the keys are all columns. The worked example below is that case.
>
...
>
> One consequence: naming a relation or an accessor in the exclusion list is now a no-op, because that key was never in the set being subtracted from. `only()` is deliberately unchanged. `HasAttributes::only()` calls `getAttribute()` per named key, which does resolve accessors and relations, so `$this->relation->only(['posts'])` still emits the relation. The two branches diverge the way Eloquent's own do.
