...
- **Model `toArray()` spreads** — `[...$user->toArray(), 'flag' => true]` inside an array literal types as `Omit<User, 'flag'> & { flag: boolean }`, intersecting the model interface with the literal's own keys instead of collapsing to `unknown[]`. The `Omit<>` is what makes the intersection mean what the PHP means, since a later key wins at runtime but `&` would collapse a collision to `never`. Three caveats, because the arm emits a *reference* to the `{Model}` interface rather than a re-derived shape: a relation loaded before the spread is in the JSON payload but not in the type (`toArray()` appends `relationsToArray()`, which is not knowable statically); `$appends` show up at runtime but live in `{Model}Mutators` under the `model-split` template, not in bare `{Model}`; and `$hidden` columns are stripped at runtime but stay in `{Model}` unless `models.exclude_hidden` is on.
